...
use orca_whirlpools_client::{accounts::{Position, TickArray, Whirlpool}, get_position_address, get_tick_array_address, instructions::{ClosePosition, ClosePositionWithTokenExtensions, CollectFeesV2, CollectFeesV2InstructionArgs, CollectRewardV2, CollectRewardV2InstructionArgs, DecreaseLiquidityV2, DecreaseLiquidityV2Builder, DecreaseLiquidityV2InstructionArgs, UpdateFeesAndRewards, UpdateFeesAndRewardsBuilder, UpdateFeesAndRewardsInstructionData}};
use orca_whirlpools_core::{collect_fees_quote, collect_rewards_quote, decrease_liquidity_quote, decrease_liquidity_quote_a, decrease_liquidity_quote_b, get_tick_array_start_tick_index, get_tick_index_in_array, CollectFeesQuote, CollectRewardsQuote, DecreaseLiquidityQuote};
...
pub fn close_position_instructions(
  ...
  let reward_infos: Vec<Option<Account>> = pool.reward_infos.iter().enumerate()
    .flat_map(|(i, x)| if x.mint == Pubkey::default() { None } else { Some(mint_infos[i + 3].clone()) })
    .collect();
  ...
  let quote = decrease_liquidity_quote(position.liquidity, slippage_tolerance_bps, pool.sqrt_price, position.tick_lower_index, position.tick_upper_index, transfer_fee_a, transfer_fee_b)?;
  ...
  let tick_array_infos = rpc.get_multiple_accounts(&[
    lower_tick_array_address,
    upper_tick_array_address,
  ])?;
  ...
  let lower_tick = &lower_tick_array.ticks[
    get_tick_index_in_array(position.tick_lower_index, lower_tick_array_start_index, pool.tick_spacing)? as usize
  ];
  ...
  let upper_tick = &upper_tick_array.ticks[
    get_tick_index_in_array(position.tick_upper_index, upper_tick_array_start_index, pool.tick_spacing)? as usize
  ];
  ...
  if quote.liquidity_delta > 0 ||
    fees_quote.fee_owed_a > 0 ||
    fees_quote.fee_owed_b > 0 {
  ...
  for i in 0..3 {
    if rewards_quote.rewards[i].rewards_owed > 0 {
      required_mints.push(TokenAccountStrategy::WithoutBalance(pool.reward_infos[i].mint));
    }
  }
  ...
  let token_owner_account_a = token_accounts.token_account_addresses.get(&pool.token_mint_a).ok_or("Token A owner account not found")?;
  let token_owner_account_b = token_accounts.token_account_addresses.get(&pool.token_mint_b).ok_or("Token B owner account not found")?;
  ...
  for i in 0..3 {
    ...
    instructions.push(
      CollectRewardV2 {
        ...
        position: position_address,
        position_token_account: position_token_account_address,
        reward_owner_account: *token_accounts.token_account_addresses.get(&pool.reward_infos[i].mint).unwrap(),
  ...
  match position_info.owner {
    ...
    _ => { return Err("Unsupported token program".into()); }
  }
