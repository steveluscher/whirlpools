...
pub const DEFAULT_WHIRLPOOLS_CONFIG_ADDRESS: Pubkey = Pubkey::new_from_array([
  19, 228, 65, 248, 57, 19, 202, 104, 176, 99, 79, 176, 37, 253, 234, 168, 135, 55, 232, 65, 16, 209, 37, 94, 53, 123, 51, 119, 221, 238, 28, 205
]);
...
pub const DEFAULT_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS: Pubkey = Pubkey::new_from_array([
  90, 182, 180, 56, 174, 38, 113, 211, 112, 187, 90, 174, 90, 115, 121, 167, 83, 122, 96, 10, 152, 57, 209, 52, 207, 240, 174, 74, 201, 7, 87, 54
]);
...
pub static WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS: Mutex<Pubkey> = Mutex::new(DEFAULT_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS);
...
pub fn set_whirlpools_config_address(address: Pubkey) -> Result<(), Box<dyn Error>> {
  ...
  *WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS.try_lock()? = get_whirlpools_config_extension_address(&address)?.0;
  Ok(())
}
...
mod tests {
    ...
    fn test_set_whirlpools_config_address() {
      ...
      let new_extension = Pubkey::from_str("Ez4MMUVb7VrKFcTSbi9Yz2ivXwdwCqJicnDaRHbe96Yk").unwrap();
      set_whirlpools_config_address(new_config).unwrap();
      assert_eq!(*WHIRLPOOLS_CONFIG_ADDRESS.lock().unwrap(), new_config);
      assert_eq!(*WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS.lock().unwrap(), new_extension);
    }
