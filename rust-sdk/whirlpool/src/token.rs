...
pub fn prepare_token_accounts_instructions(
    rpc: &RpcClient,
    owner: Pubkey,
    ...
    let native_mint_index = mint_addresses.iter().position(|&x| x == NATIVE_MINT);
    let has_native_mint = native_mint_index.is_some();

    let maybe_mint_account_infos = rpc.get_multiple_accounts(&mint_addresses)?;
    let mint_account_infos: Vec<&SolanaAccount> = maybe_mint_account_infos.iter()
        .map(|x| x.as_ref().ok_or(ProgramError::UninitializedAccount))
        .collect::<Result<Vec<&SolanaAccount>, ProgramError>>()?;

    let ata_addresses: Vec<Pubkey> = mint_account_infos.iter()
        .enumerate()
        .map(|(i, x)| get_associated_token_address_with_program_id(&owner, &x.owner, &mint_addresses[i]))
        .collect();
    ...
    let use_native_mint_ata = native_mint_wrapping_strategy == NativeMintWrappingStrategy::Ata || native_mint_wrapping_strategy == NativeMintWrappingStrategy::None;
